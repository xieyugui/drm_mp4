...
int Mp4Meta::post_process_meta() {
	...
	TSDebug(PLUGIN_NAME, "yyyyy this->content_length=%ld",
			this->content_length);
...
int Mp4Meta::mp4_get_start_sample(Mp4Trak *trak) {
	...
	if (size == 0) { //全部sample 数目，如果所有的sample有相同的长度，这个字段就是这个值，否则就是0
		for (i = 0; i < entries; i++) {
			...
			TSDebug(PLUGIN_NAME,
					"mp4_update_stsz_atom sample_size=%u, start_chunk_offset=%ld",
					sample_size, start_chunk_offset);
	...
	found: TSIOBufferReaderFree(readerp);
...
off_t Mp4Meta::mp4_get_start_chunk_offset_co64(Mp4Trak *trak) {
...
off_t Mp4Meta::mp4_get_start_chunk_offset_stco(Mp4Trak *trak) {
...
int Mp4Meta::mp4_update_stsc_atom(Mp4Trak *trak) {
	...
	for (i = 1; i < trak->sample_to_chunk_entries; i++) {  //该trak一共有多少个chunk
		...
		//debug
		TSDebug(PLUGIN_NAME, "mp4_update_stsc_atom next_chunk=%u %u %u",
				next_chunk,
				mp4_reader_get_32value(readerp,
						offsetof(mp4_stsc_entry, samples)),
				mp4_reader_get_32value(readerp, offsetof(mp4_stsc_entry, id)));
		//over debug
	...
	for (j = i; j < trak->sample_to_chunk_entries; j++) {
		chunk = mp4_reader_get_32value(readerp,
				offsetof(mp4_stsc_entry, chunk));
		TSDebug(PLUGIN_NAME,
				"mp4_update_stsc_atom cccc chunk=%lu, trak->start_chunk=%u",
